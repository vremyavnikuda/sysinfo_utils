...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("1. Getting all GPUs:");
    let gpus = manager.get_all_gpus_owned();
    println!("Found {} GPU(s)", gpus.len());
    for (i, gpu) in gpus.iter().enumerate() {
        println!("  GPU {}: {}", i, gpu.format_name_gpu());
    }
    println!("2. Accessing GPUs multiple times (should use cache):");
    ...
    println!("4. Waiting for cache expiration...");
    std::thread::sleep(Duration::from_secs(3));
    println!("5. Accessing after expiration (should refresh cache):");
    if let Some(gpu) = manager.get_gpu_cached(0) {
        println!(
            "  GPU: {} (temp: {}°C)",
            gpu.format_name_gpu(),
    ...
    println!("6. Cache statistics after expiration:");
    if let Some(stats) = manager.get_cache_stats() {
        println!("-- Total entries: {}", stats.total_entries);
        println!("-- Total accesses: {}", stats.total_accesses);
        println!("-- Oldest entry age: {:?}", stats.oldest_entry_age);
