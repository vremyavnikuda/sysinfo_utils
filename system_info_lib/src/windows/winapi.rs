...
fn version() -> (SystemVersion, Option<String>) {
    match version_info() {
        None => (SystemVersion::Unknown, None),
        Some(v) => (
            SystemVersion::Semantic(
...
mod tests {
    ...
    fn version() {
        let info = get();
        assert_eq!(Type::Windows, info.system_type());
    }
