...
struct GpuOptions {
    ...
    #[clap(short = 's', long = "power")]
    power_usage: bool,
    /// Show GPU clock speed.
    #[clap(short = 'a', long = "clock")]
    clock_speed: bool,
}
    ...
    if let Some(gpu_info) = manager.gpus.first() {
                ...
                || gpu_options.clock_speed)
        {
            ...
            println!(
                "GPU information:\n{:?}\n{}\n{}\n{}\n{}",
                gpu_info.get_name(),
                gpu_info.get_temperature(),
                gpu_info.get_utilization(),
                gpu_info.get_power_usage(),
                gpu_info.get_clock_speed()
            );
        } else {
            if gpu_options.vendor {
            ...
            if gpu_options.temperature {
                println!("GPU Temperature: {}", gpu_info.get_temperature());
            }
            if gpu_options.utilization {
                println!("GPU Utilization: {}", gpu_info.get_utilization());
            }
            if gpu_options.power_usage {
                println!("GPU Power Usage: {}", gpu_info.get_power_usage());
            }
            if gpu_options.clock_speed {
                println!("GPU Clock Speed: {}", gpu_info.get_clock_speed());
            }
        }
    }
