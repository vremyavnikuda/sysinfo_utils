...
impl GpuManager {
	...
	/// NVIDIA GPU detection and info parsing
	///
	/// Uses `nvidia-smi` to gather information about installed NVIDIA GPUs.
	///
	/// # Parameters
	/// None
	///
	/// # Returns
	/// None
	fn nvidia_info(&mut self) {
		if let Ok(output) = Command::new("nvidia-smi")
			.arg("--query-gpu=name,temperature.gpu,utilization.gpu,clocks.current.graphics,clocks.max.graphics,power.draw,power.max_limit").arg("--format=csv,noheader,nounits")
			.output() {
